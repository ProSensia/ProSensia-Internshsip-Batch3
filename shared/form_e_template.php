...
function render_form_e_document(array $d, string $mode = 'final'): void {
    ...
    // The QR/"no signature needed" treatment belongs to the Industry
    // Supervisor slot (ProSensia/Momin Khan side) ONLY — it must never
    // appear for the Academic Supervisor (Pak-Austria advisor), who always
    // gets a normal named signature line. It must also never appear unless
    // the Founder & CEO specifically approved it: a record that is
    // "issued" by any other path (e.g. a legacy record from before the
    // Founder approval stage existed) shows a red warning instead of a QR.
    $founderVerified = !empty($d['founder_approved_by_name']);
    $issuedNotFounderVerified = !$founderVerified && !empty($d['issued_at']);
    ?>
<!DOCTYPE html>
<html lang="en">
<head>
...
<div class="sheet">
  ...
  <div class="sig-row">
    <div class="sig-col">
      ...
      <?php elseif ($issuedNotFounderVerified): ?>
      <div class="sig-line" style="color:#b91c1c;font-weight:700">&#9888; Not Verified by the Founder &amp; CEO</div>
      <div style="color:#b91c1c">Please forward this document for verification.</div>
      <?php else: ?>
  ...
  <div class="fe-footer">
    <div class="doc-meta">
      ...
      <?php if ($founderVerified): ?><br><b style="color:#0a7a2f">✓ Verified — no physical signature needed, scan the QR to confirm.</b><?php endif; ?>
      <?php if ($issuedNotFounderVerified): ?><br><b style="color:#b91c1c">⚠ Not verified by the Founder &amp; CEO — please forward for verification.</b><?php endif; ?>
    </div>
    <?php if ($verifyUrl && $founderVerified): ?>
    <div class="qr">
