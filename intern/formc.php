...
if ($role === 'intern') {
  ...
  // Check if profile has all required fields for Form C
  $required_profile_fields = ['father_name', 'cnic', 'reg_number', 'department', 'semester', 'address'];
  $missing = [];
  ...
  if (!empty($missing)) {
    flash('Please complete your profile first: ' . implode(', ', $missing) . '.', 'warning');
  ...
  if ($role === 'intern') {
    ...
    $required_fields = ['father_name', 'cnic', 'reg_number', 'department', 'semester', 'address'];
    $missing = [];
  ...
  <form method="post" class="glass card-pad mt-2">
    ...
    <div class="row g-3">
      ...
      <div class="col-md-4"><label class="form-label">Department</label><input class="form-control"
          value="<?= e($profile['department'] ?? 'Electrical and Computer Engineering') ?>" readonly disabled></div>
    ...
    <div class="row g-3">
      ...
      <div class="col-md-4"><label class="form-label">Joining date <span class="text-danger">*</span></label><input
          class="form-control" type="date" name="joining_date" value="<?= e($f['joining_date'] ?? '') ?>" required></div>
      <div class="col-md-4"><label class="form-label">Start date <span class="text-danger">*</span></label><input
          class="form-control" type="date" name="start_date" value="<?= e($f['start_date'] ?? '') ?>" required></div>
    ...
    <?php foreach ($rows as $f): ?>
      <div class="py-3" style="border-top:1px solid var(--border)">
        ...
        <form method="post" class="row g-2 align-items-end mt-2">
          ...
          <div class="col-md-6"><input class="form-control form-control-sm" name="note" placeholder="Optional reviewer note"
              value="<?= e($f['reviewer_note'] ?? '') ?>"></div>
          <div class="col-md-3 d-flex gap-2"><button class="btn btn-sm btn-primary flex-grow-1">Submit decision</button><a
              class="btn btn-sm btn-outline-light" href="<?= base_url('intern/formc_pdf.php?uid=' . (int) $f['user_id']) ?>"
              target="_blank"><i class="bi bi-file-earmark-pdf"></i></a></div>
        </form>
      </div>
    <?php endforeach; ?>
  </div>
